"""Stack-slot instruction selection, ABI lowering, layout and relocation."""

from dataclasses import dataclass
from . import isa
from .model import CompileError, align_up
    ...
    def validate(self):
        for name, size in [
            ...
            if size < 4 or size > 2**32 or size & (size - 1):
                raise CompileError(f"{name} size must be a power of two from 4 to 2^32")
...
class Assembler:
    ...
    def address(self, r, label, addend=0):
        self.fixups.append((len(self.code), r, label, addend))
        self.emit(isa.constant(r, 0))
        if self.target.pic:
