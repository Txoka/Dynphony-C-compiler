...
- `python -m unittest discover -s tests -v`: **39 test methods passed**, including CFG reachability, the global Tier 1 fixed point, local and interprocedural constant collapse, single-caller relocation, startup/`main` absorption, algebraic identities, address safety, deterministic randomized arithmetic, register allocation, tail calls, device intrinsics, and fixed/PIC termination. Final run: 3.987 seconds.
