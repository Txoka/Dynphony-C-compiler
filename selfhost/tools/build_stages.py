...
def compile_stage(compiler, persistent, load_address, max_steps):
    machine = Machine(
        compiler,
...
def main(argv=None):
    ...
    parser.add_argument("--load-address", type=lambda value: int(value, 0),
                        default=0)
    parser.add_argument("--max-steps", type=int, default=1_200_000_000)
    ...
    stage1, steps1 = compile_stage(
        stage0, persistent, args.load_address, args.max_steps
    )
    stage2, steps2 = compile_stage(
        stage1, persistent, args.load_address, args.max_steps
    )
