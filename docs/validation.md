...
- `python -m unittest discover -s tests -v`: **46 test methods passed**, including package boundaries, named registers and ABI roles, canonical lowering, `_Bool`/`bool` conversions, VLA allocation and scope cleanup, structures and natural layout, self-referential structure pointers, enums, `const`, local typedefs, static locals, register and stack-passed scalar arguments, the arena allocator, direct/indirect call IR, conditional fallthrough IR, CFG reachability, the global Tier 1 fixed point, constant collapse, register allocation, tail calls, device intrinsics, and fixed/PIC termination. Final run: 3.927 seconds.
