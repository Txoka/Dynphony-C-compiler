"""Encoding primitives transcribed from docs/isa.txt. All immediates are U16."""
...
def mov(d, source, immediate=False):
    ...
    return alu("or", d, 0, source, immediate)


def push(r):
    return alu("sub", 14, 14, 4, True) + store(4, 14, r)


def pop(r):
    return load(4, r, 14) + alu("add", 14, 14, 4, True)


def call(target, immediate=False):
    return (
        counter(15)
        + alu("add", 15, 15, 17 if immediate else 16, True)
        + push(15)
